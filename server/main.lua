...
local function isResponder(source)
    if Config.Debug and Config.Access.allowEveryoneWhenDebug then
        return true
    end
...
local function getRecentCalls()
    ...
    for _, call in pairs(Calls) do
        list[#list + 1] = publicCall(call)
    end
    table.sort(list, function(a, b)
        return a.id > b.id
    end)
    while #list > Config.CallSettings.historyLimit do
        table.remove(list)
    end
    return list
end
...
local function notifyCaller(call, message, kind)
    if not Config.Notifications.notifyCaller or not call or not call.callerSource then
        return
    end
...
local function isAssignedToCall(call, source)
    return (call.primaryUnit and call.primaryUnit.source == source) or
               (call.attachedUnits and call.attachedUnits[source] ~= nil)
end
...
RegisterNetEvent('simple911:server:createCall', function(data)
    ...
    if lastCall and now - lastCall < Config.CallSettings.cooldownSeconds then
        return notify(source, Config.Messages.cooldown:format(Config.CallSettings.cooldownSeconds - (now - lastCall)),
            'error')
    end

    local message = sanitize(data.message, Config.CallSettings.maxMessageLength)
    if message == '' then
        return notify(source, Config.Messages.empty, 'error')
    end
...
RegisterNetEvent('simple911:server:respondToCall', function(callId)
    ...
    if not call then
        return notify(source, Config.Messages.invalidCall, 'error')
    end
...
RegisterNetEvent('simple911:server:markOnScene', function(callId)
    local source = source
    if not Config.OnScene.enabled or not isResponder(source) then
        return
    end

    callId = tonumber(callId)
    local call = callId and Calls[callId]
    if not call or call.status ~= 'enroute' or not isAssignedToCall(call, source) then
        return
    end

    call.status = 'onscene'
    call.onSceneBy = responderProfile(source)
    if Config.OnScene.notifyUnit then
        notify(source, Config.Messages.onScene:format(callId), 'success')
    end
...
RegisterNetEvent('simple911:server:requestCalls', function()
    local source = source
    if not isResponder(source) then
        return notify(source, Config.Messages.noPermission, 'error')
    end
