...
local function openCaller(serviceId)
    ...
    setFocus(true)
    SendNUIMessage({ action = 'openCaller', serviceId = serviceId, services = Config.Services, location = location, coords = coords, maxMessageLength = Config.CallSettings.maxMessageLength })
end

local function openDispatch()
    setFocus(true)
    SendNUIMessage({ action = 'openDispatch', calls = calls, units = units, selfUnit = selfUnit, onDuty = onDuty, settings = Config.Dispatch })
...
for _, service in ipairs(Config.Services) do RegisterCommand(service.command, function() openCaller(service.id) end, false) end
...
RegisterNUICallback('setCallsign', function(data, cb) TriggerServerEvent('simple911:server:setCallsign', data.callsign) cb({ ok = true }) end)
...
RegisterNetEvent('simple911:client:newCall', function(call)
    if Config.Notifications.newCall then notify(('Priority %s | New %s call #%s: %s'):format(call.priority, call.serviceId, call.id, call.title), 'emergency') end
end)
